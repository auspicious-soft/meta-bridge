...
// ✅ Use public folder paths (these are fast via Next.js static serving/CDN)
const DESKTOP_VIDEO = "/metabridge-video-optimized.mp4";
const MOBILE_VIDEO = "/metabridge-video-mobile-optimized.mp4";
const POSTER_DESKTOP = "/metabridge-video-poster.png";
const POSTER_MOBILE = "/metabridge-poster-mobile.png";
...
export default function VideoScrubSection({ t }: Props) {
  ...
  // ✅ Lazy-load video source after first paint to improve LCP
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // ✅ Update video time on scroll (scrubbing effect)
    const updateVideoTime = () => {
      if (!video || video.readyState < 2) return;
      const scrollTop = window.scrollY - container.offsetTop;
      const scrollRange = container.offsetHeight - window.innerHeight;
    ...
    // ✅ Check if video has enough data to render first frame
    const canRenderFirstFrame = () => {
      try {
        const buffered = video.buffered;
        if (buffered && buffered.length > 0) {
    ...
    // ✅ Simplified: For mobile, don't autoplay, just mark ready when metadata is available
    const onLoadedMeta = () => {
      if (window.innerWidth < 768) {
        markReady();
      } else if (canRenderFirstFrame()) {
        markReady();
      }
    };
    ...
    <div ref={containerRef} className="relative" style={{ height: "300vh" }}>
      <div className="sticky top-0 h-screen w-full overflow-hidden bg-black">
        ...
        <video
          ...
          disablePictureInPicture
          poster={POSTER_DESKTOP}
          src={window.innerWidth >= 768 ? videoSrc : ""}
        />
